...
*/
	...
	public Celula ToFila (No no){
		Celula tmp = null;
		Celula topo = ToFila(raiz, tmp);
		tmp = null;
		Celula primeiro = PilhaParaFila (topo);
		return primeiro;
	}          

	public Celula ToFila (No no, Celula topo ){
		if ( no != null ){
			topo = ToFila(no.esq, topo);
			Celula tmp = new Celula (no.elemento);
			tmp.prox = topo;
			topo = tmp;
			tmp = null;
			topo = ToFila (no.dir, topo);
		}
		return topo;
	}	

	public Celula PilhaParaFila (Celula topo){
	...
	public static void mostrarFila (Celula primeiro){
