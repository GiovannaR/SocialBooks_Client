/**
*Tp6
*Questao04 - Quicksort com Lista dinamica duplamente encadeada
...
/**
*Construtor alternativo
*@param x Pessoa - Pessoa a ser o elemento da CelulaDupla
*/
...
/**
*Metodo de ordenacao que coloca um pivo como padrao para separar o array de menores e de maiores, e ordena tudo por partes ate' tudo ficar ordenado
*@param esq CelulaDupla - o ponteiro, a partir do qual comeca a ordenacao
*@param dir CelulaDupla - o ponteiro que finaliza a ordenacao
*/ 
	public void quicksort (CelulaDupla esq, CelulaDupla dir){
		CelulaDupla i = esq;
		CelulaDupla j = dir;
		//Os comparadores marcam a posicao da CelulaDupla esq e da dir, respectivamente. Os contadores marcam as posicoes de i e j, respectivamente
		int comparador1 = 0;
		// tamanho( ) - 1 ,pois as posicoes comecam no 0
		int comparador2 = tamanho( ) - 1;
		int contador1, contador2;
		int onde = 0;

		CelulaDupla meio = esq;
		Pessoa pivo = new Pessoa ( );

		for ( CelulaDupla k = esq; k != dir.prox; k = k.prox ){
			onde ++;
		}
		
		for (int f = 0; f < (onde/2); f ++, meio = meio.prox );
		pivo = meio.elemento.clone( );

		// Se esq.ant.ant == null, o ponteiro ja' corresponde a posicao 0
		if ( esq.ant.ant != null ){
			for ( CelulaDupla n = primeiro.prox; n != esq; n = n.prox, comparador1 ++ ); 
		}

		//Se dir.prox == null, o ponteiro ja' corresponde a posicao 39
		if ( dir.prox != null ){
			for ( CelulaDupla l = ultimo; l != dir; l = l.ant, comparador2 -- );
		}

		contador1 = comparador1;
		contador2 = comparador2; 
		
		while ( (contador1 <= contador2) ){
		
			while ( ((pegarAno(i.elemento.getDataN( )) < pegarAno(pivo.getDataN( ))) || idMaior(pivo, i.elemento)) ){
				compara ++;
				i = i.prox;
				contador1 ++;
			}

			while ( (pegarAno(j.elemento.getDataN( )) > pegarAno(pivo.getDataN( ))) || idMaior( j.elemento, pivo ) ){
				compara ++;
				j = j.ant;
				contador2 --;
			}
		...
		//Mais duas comparacoes de quando a condicao do while e' falsa e sai dele
		compara += 2;

		if ( comparador1 < contador2 ){
			quicksort( esq, j );
		}
		if ( contador1 < comparador2 ){
			quicksort( i, dir );
		}
	}

/**
*Troca de elemento entre duas CelulasDuplas
*@param a CelulaDupla - CelulaDupla a ter elemento trocado
*@param b CelulaDupla - CelulaDupla a ter elemento trocado
*/
	public void swap ( CelulaDupla a, CelulaDupla b ){
		movimenta += 3;
		Pessoa aux = new Pessoa ( );
		aux = a.elemento.clone( );
		a.elemento = b.elemento.clone( );
		b.elemento = aux.clone( );
	}
		
/**
*Metodo para se o nao for igual, verificar o id
...
*/
	public boolean idMaior ( Pessoa a, Pessoa b ){
		return (pegarAno(a.getDataN( )) == pegarAno(b.getDataN( ))) && (a.getId( ) > b.getId( ));
	}
...
/**
*Funcao que retorna quantos CelulasDuplas estao encadeadas 
*@return quantidade int - quantos CelulasDuplas estao encadeadas 
*/
	public int tamanho ( ){
		int quantidade = 0;
		//O try catch e' necessario caso tenha nenhuma posicao
		try{
...
/**
*Metodo para pegar a primeira CelulaDupla sem contar com o no' cabeca
*@return CelulaDupla - CelulaDupla que corresponde a primeira posicao
*/		
...
/**
*Metodo para pegar a ultima CelulaDupla
*@return CelulaDupla - CelulaDupla que corresponde a ultima posicao
*/
...
/**
*Metodo para inicializar o metodo recursivo de imprimir os elementos contidos nas CelulasDuplas
*/		
	public void imprimir ( ){
		imprimir(primeiro.prox);
	}

/**
*Metodo recursivo para imprimir todos os elementos que estao nas CelulasDuplas
*@param i CelulaDupla - CelulaDupla cujo elemento sera' imprimido
*/		
	public void imprimir(CelulaDupla i){
		//Condicao para ver se chegou ao fim dos ponteiros
		if ( i != null ){
...
class TP06Q04QuicksortcomDupla{
...
*/
	public static void fazerArquivo ( long inicio, long fim, int comparacoes, int movimentacoes ){
		Arq.openWrite("553285_quicksort2.txt");
	...
	public static void main ( String [ ] args ){
		...
		lista.quicksort (lista.getPrimeiro( ), lista.getUltimo( ));
		lista.imprimir( );
		long fim = now ( );
