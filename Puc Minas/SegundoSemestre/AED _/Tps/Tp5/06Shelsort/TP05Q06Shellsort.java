...
/**
*Metodo que divide o array em partes para fazer o metodo de insercao 
*/
...
/**
*Faz a insercao em apenas determinada parte do array
*@param cor int - numero da parte do array que sera'ordenada
*@param h int - quantas vezes o array foi dividido
*/
	public void inserirPorCor ( int cor, int h ){
		for ( int i = (h + cor); i < n; i++ ){
			Pessoa tmp = array[i].clonar( );
			movimenta++;
			int j = i - h;
			while ( (j >= 0) && (array[j].getDataN( ).compareTo(tmp.getDataN( )) > 0) ){
				compara ++;
				array[j + h] = array[j].clonar( );
				movimenta ++;
				j -= h;
			}
			array[j + h] = tmp.clonar( );
			movimenta ++;
		}
		compara ++;
	}
...
*/
	public static void fazerArquivo ( long inicio, long fim, int comparacoes, int movimentacoes ){
		Arq.openWrite("553285_Shellsort.txt");
