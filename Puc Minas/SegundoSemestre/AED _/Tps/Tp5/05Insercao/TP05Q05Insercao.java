...
/**
*Metodo de ordenacao que compara um elemento especifico com seus anteriores no array e os desloca ate' a posicao na qual o menor deve ser colocado ser liberada
*/
	public void insercao ( ){
		for ( int i = 1; i < n; i ++ ){
			Pessoa tmp = array[i].clonar( );
			movimenta++;
			int j = i - 1;
			while ( (j >= 0) && (array[j].clonar( ).getNome( ).compareTo( tmp.getNome( ) ) > 0) ){
				compara ++;
				array[j + 1] = array[j].clonar( );
				movimenta ++;
				j --;
			}
			array[j + 1] = tmp.clonar( );
			movimenta ++;
		}
		//Ira' comparar mais uma vez, pois existe tambem a comparacao para sair do while
		compara ++;
	}	
...
*/
	public static void fazerArquivo ( long inicio, long fim, int comparacoes, int movimentacoes ){
		Arq.openWrite("553285_Insercao.txt");
