...
*@param esq int - onde comeca o array a ser ordenado
*@param dir int - onde termina o array a ser ordenado
*/ 
	public void quicksort ( int esq, int dir ) {
		int i = esq;
		int j = dir;
		Pessoa pivo = array[(dir + esq)/2].clonar( );
		while ( i <= j ){
			while ( (pegarAno(pivo.getDataN( )) > pegarAno(array[i].getDataN( ))) || idMaior( pivo, array[i]) ){
				compara ++;
				i ++;
			}
			while ( (pegarAno(array[j].getDataN( )) > pegarAno(pivo.getDataN( ))) || idMaior( array[j], pivo ) ){
				compara ++;
				j --;
			}
			if ( i <= j ){
...
*/
	public static void fazerArquivo ( long inicio, long fim, int comparacoes, int movimentacoes ){
		Arq.openWrite("553285_Quicksort.txt");
	...
	public static void main ( String [ ] args ){
		...
		lista.quicksort(0, quantidade - 1);
		lista.imprimirArray( );
		long fim = now ( );
