...
/**
*Metodo de ordenacao que faz um array de contagem que e' usado para ordenar
*/
	public void countingsort( ){
		int [ ] count = new int[Math.abs(getMaior( )) + Math.abs(getMenor( )) + 1 ];
		int menor = Math.abs(getMenor( ));
		Pessoa [ ] ordenado = new Pessoa [n];
		for ( int i = 0; i < count.length; count[i] = 0, i++ );

		for ( int i = 0; i < n; count[ doidao(array[i]) + menor ]++, i++);

		for ( int i = 1; i < count.length; count[i] += count[i - 1], i ++ );

		for ( int i = n - 1; i >= 0; ordenado[count[ doidao(array[i]) + menor ] - 1 ] = array[i].clonar( ), count[ doidao(array[i]) + menor ]--, movimenta++, i--);

		for ( int i = 0; i < n; array[ i ] = ordenado[i].clonar( ), movimenta++, i ++ );
	}	
...
/**
*Metodo que retorna o algarismo doidao para numeros 
*OBS: Optei por usar o algorismo doidao, pois ele ja' leva o id em consideracao e facilita o trabalho
*@param a Pessoa - Pessoa a ser estraida o algarismo doidao
*@return int - o algarismo doidao
*/
	public int doidao ( Pessoa a ){
		int ano = pegarAno ( a.getDataN( ) );
...
/**
*Metodo para achar o maior numero positivo 
*@return max int - maior numero positivo
*/
	public int getMaior( ){
		int max;
		max = doidao(array[0]);
...
*/
	public static void fazerArquivo ( long inicio, long fim, int comparacoes, int movimentacoes ){
		Arq.openWrite("553285_Countingsort.txt");
