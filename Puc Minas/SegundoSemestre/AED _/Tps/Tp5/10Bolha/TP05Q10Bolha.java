/**
*Tp5
*Questao05 - Ordenacao por Insercao
*
...
/**
*Metodo para tirar os espacos vazios de uma String
*/
	public void tirarEspacos ( ){
		for ( int i = 0; i < n; i++ ){
			String nome = array[i].clonar( ).getNome( );
			String semespaco = nome.trim( );
		}
	}

/**
*Metodo de ordenacao que comeca de tras para frente e o elemento especifico do array e' trocado com os que sao maiores que ele
*/
	public void bolha ( ){
    		for( int i = n - 1; i >= 1; i-- ){
        		for( int j = 0; j < i ; j++){
				compara ++;
            			if( array[j].getLocalN( ).compareToIgnoreCase( array[j + 1].getLocalN( )) > 0 || idMaior(array[j],array[j + 1])){
					movimenta += 3; 
                			Pessoa aux = array[j].clonar( );
                			array [j] = array [j + 1].clonar( );
                			array [j + 1] = aux.clonar( );
            			}
        		}
    		}
	}
...
/**
*Se o local for o mesmo, ver qual tem maior id
*@param a Pessoa - Pessoa a ter o maior id
*@param b Pessoa - Pessoa a ter o menor id
*@return boolean - se a tem um id maior que b, caso os locais de nascimento sejam iguais
*/
	public boolean idMaior (Pessoa a, Pessoa b){
		return (a.getLocalN( ) == b.getLocalN( )) && (a.getId( ) > b.getId( ));
	}	
...
*/
	public static void fazerArquivo ( long inicio, long fim, int comparacoes, int movimentacoes ){
		Arq.openWrite("553285_Bolha.txt");
